 ...
 *
 * Portable: every handle derives from the KERF_SLUG constant, so a re-skin needs no
 ...
 *
 * Cache-buster is each file's modification time, not the KERF_VERSION constant, so a
 * CSS edit busts caches immediately during iteration without a version bump.
 */
function kerf_enqueue_assets(): void {
...
add_action( 'wp_enqueue_scripts', 'kerf_enqueue_assets' );
 ...
 * an off-origin fetch.
 */
function kerf_preload_fonts(): void {
	/**
	 ...
	 * @since 1.0.0
	 */
	$fonts = (array) apply_filters( KERF_SLUG . '/preload_fonts', array() );
	...
	foreach ( $fonts as $font ) {
		...
		printf(
			...
			esc_url( KERF_URI . '/' . ltrim( $font, '/' ) )
		);
	}
}
add_action( 'wp_head', 'kerf_preload_fonts', 1 );
