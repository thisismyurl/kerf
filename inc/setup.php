 ...
 */
function kerf_setup(): void {

	// i18n. The domain is the literal 'kerf' (a constant would break
	// make-pot — see bootstrap.php); the path uses KERF_DIR so it travels with a
	// re-skin. The CLI rewrites the literal when it generates a theme.
	load_theme_textdomain( 'kerf', KERF_DIR . '/languages' );
	 ...
	 */
	$GLOBALS['content_width'] = (int) apply_filters( KERF_SLUG . '/content_width', 720 );
	...
	register_nav_menus(
		(array) apply_filters(
			KERF_SLUG . '/register_nav_menus',
			array(
				'primary' => esc_html__( 'Primary Navigation', 'kerf' ),
				'footer'  => esc_html__( 'Footer Navigation', 'kerf' ),
			)
		)
	);
	 ...
	 * @since 1.0.0
	 */
	do_action( KERF_SLUG . '/setup' );
}
add_action( 'after_setup_theme', 'kerf_setup' );
 ...
 * plugin is active.
 */
function kerf_woocommerce_support(): void {
...
add_action( 'after_setup_theme', 'kerf_woocommerce_support' );
 ...
 */
function kerf_editor_styles(): void {
	add_editor_style( array( 'assets/css/editor-style.css' ) );
}
add_action( 'after_setup_theme', 'kerf_editor_styles' );
 ...
 */
function kerf_comment_form_field_attributes( array $fields ): array {
	$attributes = array(
