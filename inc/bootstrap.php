 ...
 *
 * This is the ONE place a theme's identity lives. Change the `kerf_` function
 * prefix and the KERF_* constants and the entire theme re-prefixes, because
 ...
 * them honest.
 *
 * Why a `kerf_` function prefix and not a namespace (reversed 2026-07-20):
 ...
 * rewrites `kerf_` → `<slug>_`, `KERF_` → `<SLUG>_`, and `Kerf_` →
 ...
 * stays a string LITERAL ('kerf'), never the KERF_SLUG constant. `wp i18n
 ...
 */
define( 'KERF_VERSION', '1.6265.1511' );

/**
 * Absolute filesystem path to the theme root (no trailing slash).
 */
define( 'KERF_DIR', get_template_directory() );

/**
 * Public URL to the theme root (no trailing slash).
 */
define( 'KERF_URI', get_template_directory_uri() );
