 ...
 * from the theme, supplied through the `KERF_SLUG . '/get_started_content'` filter
 ...
 */
define( 'KERF_WELCOME_FLAG', 'kerf_welcome_notice' ); // hardcoded slug — CLI rewrites on re-skin.

/**
 * Admin-post action and nonce action string for dismissing the welcome notice.
 */
define( 'KERF_DISMISS_ACTION', 'kerf_dismiss_welcome' ); // hardcoded slug — CLI rewrites on re-skin.

/**
 * Admin-menu slug for the Get-started page.
 */
define( 'KERF_GET_STARTED_SLUG', 'kerf-get-started' );
 ...
 */
function kerf_flag_welcome_notice(): void {
	add_option( KERF_WELCOME_FLAG, '1', '', 'no' );
}
add_action( 'after_switch_theme', 'kerf_flag_welcome_notice' );
 ...
 */
function kerf_clear_welcome_notice(): void {
	delete_option( KERF_WELCOME_FLAG );
}

/**
 * Register the "Get started" page under Appearance.
 *
 * The required capability comes from kerf_get_onboarding_capability() — default is
 * edit_theme_options (users who can switch themes and configure global styles),
 * filterable via KERF_SLUG . '/onboarding_capability'.
 */
function kerf_register_get_started_page(): void {
	add_theme_page(
		/* translators: %s: theme name. */
		sprintf( esc_html__( '%s: Get started', 'kerf' ), kerf_get_theme_name() ),
		kerf_get_theme_name(),
		kerf_get_onboarding_capability(),
		KERF_GET_STARTED_SLUG,
		'kerf_render_get_started_page'
	);
}
add_action( 'admin_menu', 'kerf_register_get_started_page' );
 ...
 */
function kerf_get_theme_name(): string {
 ...
 */
function kerf_get_onboarding_capability(): string {
	/**
	 ...
	 */
	return (string) apply_filters( KERF_SLUG . '/onboarding_capability', 'edit_theme_options' ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
}
 ...
 */
function kerf_enqueue_get_started_assets( string $hook_suffix ): void {
	if ( 'appearance_page_' . KERF_GET_STARTED_SLUG !== $hook_suffix ) {
		return;
	}

	$sheet = KERF_DIR . '/assets/css/admin-get-started.css';
	if ( ! file_exists( $sheet ) ) {
		return;
	}

	wp_enqueue_style(
		KERF_SLUG . '-get-started',
		KERF_URI . '/assets/css/admin-get-started.css',
		array(),
		(string) filemtime( $sheet )
	);
}
add_action( 'admin_enqueue_scripts', 'kerf_enqueue_get_started_assets' );

/**
 * Show the dismissible welcome notice while the flag is set.
 *
 * Shown only to users who pass kerf_get_onboarding_capability(), and never on the Get-started
 ...
 */
function kerf_render_welcome_notice(): void {
	if ( ! current_user_can( kerf_get_onboarding_capability() ) ) {
		return;
	}

	if ( '1' !== get_option( KERF_WELCOME_FLAG ) ) {
		return;
	}

	$screen = get_current_screen();
	if ( $screen instanceof \WP_Screen && 'appearance_page_' . KERF_GET_STARTED_SLUG === $screen->id ) {
		return;
	}

	$page_url = admin_url( 'themes.php?page=' . KERF_GET_STARTED_SLUG );
	$theme    = kerf_get_theme_name();
	$dismiss  = wp_nonce_url( admin_url( 'admin-post.php?action=' . KERF_DISMISS_ACTION ), KERF_DISMISS_ACTION );
	?>
	<div class="notice notice-info is-dismissible kerf-welcome-notice">
		<p>
			<strong>
				<?php
				/* translators: %s: theme name. */
				printf( esc_html__( 'Welcome to %s.', 'kerf' ), esc_html( $theme ) );
				?>
			</strong>
			<?php esc_html_e( 'Thanks for giving it a home. A few small steps will have your site reading the way it should.', 'kerf' ); ?>
		</p>
		<p>
			<a class="button button-primary" href="<?php echo esc_url( $page_url ); ?>">
				<?php
				/* translators: %s: theme name. */
				printf( esc_html__( 'Set up %s →', 'kerf' ), esc_html( $theme ) );
				?>
			</a>
			<a class="kerf-welcome-notice__dismiss" href="<?php echo esc_url( $dismiss ); ?>">
				<?php esc_html_e( 'Dismiss', 'kerf' ); ?>
			</a>
		</p>
	</div>
	<?php
}
add_action( 'admin_notices', 'kerf_render_welcome_notice' );
 ...
 */
function kerf_handle_welcome_dismiss(): void {
	if ( ! current_user_can( kerf_get_onboarding_capability() ) ) {
		wp_die( esc_html__( 'You do not have permission to do that.', 'kerf' ) );
	}

	check_admin_referer( KERF_DISMISS_ACTION );
	kerf_clear_welcome_notice();
	wp_safe_redirect( admin_url() );
	exit;
}
add_action( 'admin_post_' . KERF_DISMISS_ACTION, 'kerf_handle_welcome_dismiss' );

/**
 * The Get-started page content.
 *
 * Core supplies a plain, honest default so the page is coherent out of the box.
 * A theme overrides any part of it through the `KERF_SLUG . '/get_started_content'`
 ...
 */
function kerf_get_started_content(): array {
	$theme = kerf_get_theme_name();
	...
	$default = array(
		 ...
		 * echo in kerf_render_get_started_page() (the 'title' at the <strong>, the
		...
		'steps'      => array(
			array(
				'title' => __( 'Choose what people land on.', 'kerf' ),
				'body'  => esc_html__( "A visitor's first second decides whether they stay. Set a static front page under Settings → Reading, and give your posts a page of their own.", 'kerf' ),
			),
			array(
				'title' => __( 'Give people a way around.', 'kerf' ),
				'body'  => esc_html__( 'A site without a menu is a room without doors. Open the Site Editor, edit the header, and assign your menu to Primary Navigation.', 'kerf' ),
			),
			array(
				'title' => __( 'Start from a pattern, not a blank page.', 'kerf' ),
				'body'  => esc_html__( 'In any page or post, open the block inserter, choose Patterns, and find this theme\'s group. Drop one in and change the words.', 'kerf' ),
			),
			array(
				'title' => __( 'Make it sound like you.', 'kerf' ),
				'body'  => esc_html__( 'In the Site Editor, open Styles to change colours and typefaces. Nothing you do there can break the theme — experiment freely.', 'kerf' ),
			),
		),
		'optimize'   => array(
			esc_html__( "This theme is fast by design: reading is JavaScript-free (only the breaking-news dismiss control ships a small script), self-hosted fonts that don't phone home, and tuning against the Core Web Vitals search engines actually measure.", 'kerf' ),
		...
		'developers' => array(
			...
			'text'  => __( 'This theme is built on Colophon, a small documented core meant to be reused. The %s walks through how to build your own theme on it.', 'kerf' ),
			'url'   => apply_filters( KERF_SLUG . '/developer_guide_url', 'https://thisismyurl.com/colophon/' ), // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
			'label' => __( 'developer guide', 'kerf' ),
		),
	);
	 ...
	 *
	 * A theme replaces any key with its own voice. See kerf_get_started_content() for
	 * the array shape.
	 *
	 * @since 1.0.0
	 *
	 ...
	 */
	return (array) apply_filters( KERF_SLUG . '/get_started_content', $default ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
}
 ...
 */
function kerf_render_get_started_page(): void {
	if ( ! current_user_can( kerf_get_onboarding_capability() ) ) {
		return;
	}
	...
	$theme   = kerf_get_theme_name();
	$content = kerf_get_started_content();
	...
	?>
	<div class="wrap kerf-get-started">

		<h1>
			<?php
			/* translators: %s: theme name. */
			printf( esc_html__( '%s: Get started', 'kerf' ), esc_html( $theme ) );
			?>
		</h1>

		<?php if ( ! empty( $content['lead'] ) ) : ?>
			<p class="kerf-get-started__lead"><?php echo wp_kses( $content['lead'], $cl_inline ); ?></p>
		<?php endif; ?>

		<?php if ( ! empty( $content['steps'] ) && is_array( $content['steps'] ) ) : ?>
			<h2><?php esc_html_e( 'Set up', 'kerf' ); ?></h2>
			<ol class="kerf-get-started__steps">
		...
		<?php if ( ! empty( $content['optimize'] ) && is_array( $content['optimize'] ) ) : ?>
			<h2><?php esc_html_e( 'Optimize', 'kerf' ); ?></h2>
		...
		<?php if ( ! empty( $content['developers']['text'] ) ) : ?>
			<h2><?php esc_html_e( 'For developers', 'kerf' ); ?></h2>
			<p>
				<?php
