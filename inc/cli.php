 ...
 *
 * Registers:
 *   wp kerf version  — theme name and version, read from KERF_SLUG + KERF_VERSION.
 *   wp kerf info     — name, version, active template, .pot presence.
 *   wp kerf flush    — wp_cache_flush().
 *
 * @package kerf
 */
...
class Kerf_CLI_Command {
	...
	public function version(): void {
		$theme = wp_get_theme();
		\WP_CLI::log( sprintf( '%s %s', (string) $theme->get( 'Name' ), KERF_VERSION ) );
	}
	...
	public function info(): void {
		...
		$languages_dir = KERF_DIR . '/languages';
		$pot_path      = $languages_dir . '/' . KERF_SLUG . '.pot';
		$has_pot       = is_dir( $languages_dir ) && file_exists( $pot_path );

		\WP_CLI::log( 'Name:            ' . (string) $theme->get( 'Name' ) );
		\WP_CLI::log( 'Slug:            ' . KERF_SLUG );
		\WP_CLI::log( 'Version:         ' . KERF_VERSION );
		\WP_CLI::log( 'Active template: ' . $template );
		\WP_CLI::log( 'languages/.pot:  ' . ( $has_pot ? 'present' : 'missing — run: wp i18n make-pot . languages/' . KERF_SLUG . '.pot' ) );
	}
...
\WP_CLI::add_command( 'kerf', 'Kerf_CLI_Command' );
