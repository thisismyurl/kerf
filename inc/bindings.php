 ...
 * copyright paragraph to the `kerf/copyright` source registered here (WP
 ...
 * names derive from KERF_SLUG, so a theme installed beside its siblings never
 ...
 */
function kerf_register_bindings(): void {
	...
	register_block_bindings_source(
		KERF_SLUG . '/copyright',
		array(
			'label'              => esc_html__( 'Copyright line', 'kerf' ),
			'get_value_callback' => 'kerf_get_copyright_value',
			'uses_context'       => array(),
		)
	);

	register_block_bindings_source(
		KERF_SLUG . '/site-strings',
		array(
			'label'              => esc_html__( 'Theme strings', 'kerf' ),
			'get_value_callback' => 'kerf_get_site_string_value',
			'uses_context'       => array(),
		)
	);
...
add_action( 'init', 'kerf_register_bindings' );
 ...
 */
function kerf_get_site_string_value( array $source_args ): string {
	switch ( $source_args['key'] ?? '' ) {
		case 'breaking-label':
			return esc_html__( 'Breaking', 'kerf' );
		case 'home-label':
			return esc_html__( 'Back to the front page', 'kerf' );
		case 'home-url':
			return esc_url( home_url( '/' ) );
		case 'not-found-title':
			return esc_html__( 'Page not found', 'kerf' );
		case 'posts-title':
			$posts_page = (int) get_option( 'page_for_posts' );
			return $posts_page ? esc_html( get_the_title( $posts_page ) ) : esc_html__( 'Latest news', 'kerf' );
	}
	return '';
}
 ...
 */
function kerf_get_copyright_value(): string {
	/**
	 ...
	 */
	$format = (string) apply_filters( KERF_SLUG . '/copyright_date_format', 'Y' );
	 ...
	 */
	$copyright = (string) apply_filters( KERF_SLUG . '/copyright_text', $copyright );
 ...
 */
function kerf_get_publication_date_value(): string {
	/**
	 ...
	 */
	$format = (string) apply_filters( KERF_SLUG . '/publication_date_format', (string) get_option( 'date_format' ) ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
